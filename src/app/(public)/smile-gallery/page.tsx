    ...
    <>
      ...
      <div className="container-page pb-8">
        ...
        <p className="prose-clinic mt-4">
          Real treatment results from patients of this practice. Every case here is published
          with that patient&apos;s written, specific consent, and any patient can withdraw
          that consent at any time — in which case their case disappears from this page.
        </p>
      </div>
      ...
      <div className="container-page">
        <ul className="flex flex-wrap gap-2">
          {GALLERY_CATEGORIES.map((item) => (
            <li key={item.slug}>
              <Link
                href={item.slug === "all" ? "/smile-gallery" : `/smile-gallery?category=${item.slug}`}
                className={cn(
      ...
      <Section className="pt-10">
        <div className="container-page">
          ...
          <div className="mt-10 flex items-start gap-3 rounded-[--radius-card] border border-[--color-hairline] bg-[--color-surface-sunken] p-6">
            <ShieldCheck className="mt-0.5 size-5 shrink-0 text-[--color-accent]" aria-hidden="true" />
            <div className="text-sm leading-relaxed text-[--color-ink-muted]">
              ...
              <p className="mt-2">
                Photographs are not retouched to change the result. Lighting and angle are
                matched between before and after as closely as clinical photography allows.
              </p>
            </div>
          </div>
        </div>
